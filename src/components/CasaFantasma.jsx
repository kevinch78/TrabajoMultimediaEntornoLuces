...
const CasaEscena = () => {
  ...
  useEffect(() => {
    ...
    // Grave (usaremos su textura para las cruces)
    ...
    /**
     * Crosses - cruces en lugar de tumbas
     */
    const createCross = (material) => {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      const cross = createCross(crossMaterial);
    ...
    gltfLoader.load("/public/models/ghost.glb", (gltf) => {
      const ghostModel = gltf.scene;
      ghostModel.scale.set(0.10, 0.5, 0.5); // ajusta tamaño
      ghostModel.position.set(10, 0, 0); // posición inicial
      scene.add(ghostModel);

      // Animación simple de movimiento flotante
      const clock = new THREE.Clock();
      const animateGhost = () => {
        const elapsedTime = clock.getElapsedTime();
        ghostModel.position.y = Math.sin(elapsedTime * 2) * 0.5 + 1;
        ghostModel.rotation.y = elapsedTime * 0.8;
